...
void spi_master_handle(uint8_t mosi_cmd, char *output_buffer) {
    ...
    LATGbits.LATG9 = 0;                     //Set CS to begin SPI comm (active low)
    ...
    switch (mosi_cmd) {
        ...
        case 'D':
            ...
            break;
        case 'E':
            bytes_to_read = 2;              // Gesture data (2 bytes)
            break;
        default:
            bytes_to_read = 2;              // Default case (2 bytes)
    ...
    for (buff_idx = 1; buff_idx < bytes_to_read + 1; buff_idx++) {
        delay_nop(2000);
        // Store Slave response in buffer (start in second position)
        output_buffer[buff_idx] = spi_master_rw(DUMMY);
    }
    
    LATGbits.LATG9 = 1;    // Terminate SPI comm
